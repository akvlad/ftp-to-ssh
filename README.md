...
| fileSlice         | String  | dd \"if={{FROM}}\" \"of={{TO}}\" bs={{SLICE_SIZE}} count={{SLICE_COUNT}} skip={{SLICE_OFFSET}} | Command to copy part of a remote file. <br/> `{{FROM}}` - name of a file, <br/> `{{TO}}` - where to copy <br/> `{{SLICE_SIZE}}` - size of a chunk <br /> `{{SLICE_COUNT}}` - number of chunks to copy <br /> `{{SLICE_OFFSET}}` - number of chunks to offset from the file start <br/> `{{SIZE}}` - number of bytes to copy (SLICE_SIZE * SLICE_COUNT) <br /> `{{OFFSET}}` - number of bytes to skip from the start (SLICE_SIZE * SLICE_OFFSET) |
